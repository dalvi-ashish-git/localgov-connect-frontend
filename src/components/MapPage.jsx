...
const createCustomIcon = (color) => {
    return new L.DivIcon({
        ...
        iconSize: [32, 32],
        iconAnchor: [16, 32], // Pin ki نوک sahi jagah par rahegi
        popupAnchor: [0, -32] // Popup pin ke upar khulega
    });
};
    ...
    async function fetchIssues() {
        ...
        try {
            ...
            if (data) {
                ...
                setFilteredIssues(issuesWithLocation); // Shuruaat mein saare issues dikhayein
            }
        } catch (error) {
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg h-full flex flex-col p-4 gap-4">
            ...
            <div className="flex items-center gap-2">
                {['All Issues', 'Resolved', 'Pending', 'Rejected'].map(filter => (
                    <button 
                        key={filter}
                        onClick={() => setActiveFilter(filter)}
                        className={`px-4 py-2 text-sm font-semibold rounded-full ${activeFilter === filter ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
                    >
                        {filter}
                    </button>
                ))}
            ...
            {/* Map Container */}
            <div className="flex-grow h-full w-full rounded-xl overflow-hidden">
                <MapContainer center={[19.45, 72.8]} zoom={11} style={{ height: '100%', width: '100%' }}>
                    ...
                    {filteredIssues.map(issue => {
                        ...
                        const icon = icons[issue.status] || createCustomIcon('gray'); // Default icon agar status match na ho
                        return (
